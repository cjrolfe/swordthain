...
- **`ActivityLogTable`** (`swordthain-activity-log`) — `logId` PK, GSIs `byUser` and `byFolder` (both + `timestamp`). Write-only for now: `MediaAccessFn` logs every view/download, matching the spec's `GET /media/{id}/view-url` / `download-url` behavior exactly ("logs 'view'" / "logs 'download'"). The owner-facing dashboard to actually query this ("per-album and per-item activity, filterable by friend") is Phase 6 — the GSIs are added now since retrofitting them onto existing data later is more awkward than adding them upfront.
