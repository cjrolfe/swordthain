...
(function () {
  ...
  // Group endpoints by folder (if any have one) for <optgroup>s in the select.
  ...
  for (const [folder, endpoints] of groups) {
    const parent = folder
  ...
  function currentEndpoint() {
    return config.endpoints.find((e) => e.id === endpointSelect.value);
  }
  ...
  formEl.addEventListener("submit", async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api-testing`, {
        ...
        body: JSON.stringify({ provider: config.provider, endpointId: ep.id, params }),
      });
